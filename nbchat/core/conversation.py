...
class ConversationMixin:
    ...
    def _run_conversation_loop(self, client) -> None:
        ...
        self._clean_turns = 0
        # Consecutive-tool-error counter for adaptive-effort escalation.
        # Must be initialised here: it was previously only assigned inside
        # the stall / non-error branches, so a tool error on the very first
        # tool turn hit `_error_streak += 1` before assignment and raised
        # UnboundLocalError, killing the conversation loop.
        _error_streak = 0
        ...
        for turn in range(self.MAX_TOOL_TURNS + 1):
            ...
            # Phase 1c: execute this turn's tool calls up front, batching
            # maximal runs of consecutive read-only reads in parallel.
            # The returned list is in ORIGINAL tool-call order (identical to
            # a serial loop), so we just consume it one at a time below.
            _turn_results = self._batch_tool_results(tool_calls)
            ...
            for tc in tool_calls:
                ...
                try:
                    # Phase 1c: raw_result was already produced (possibly
                    # concurrently with sibling reads) by _batch_tool_results.
                    _tc_r, _tn_r, _ta_r, raw_result, _he_r = _turn_results.pop(0)
                finally:
